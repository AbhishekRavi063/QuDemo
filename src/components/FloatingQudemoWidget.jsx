...
const FloatingQudemoWidget = ({
  ...
  useEffect(() => {
    console.log("🚀 FloatingQudemoWidget MOUNTED", {
      qudemoId,
      companyName,
      isPreview,
    });
    ...
    return () => {
      console.log("💀 FloatingQudemoWidget UNMOUNTED", {
        qudemoId,
        companyName,
      });
    };
  ...
  useEffect(() => {
    ...
    if (
      ...
      !introVideoPreview &&
      !qudemoData &&
      !hasAttemptedDirectFetchRef.current
    ) {
      // Validate that companyName and qudemoId are valid strings
      if (
        typeof companyName !== "string" ||
        typeof qudemoId !== "string" ||
        !companyName.trim() ||
        !qudemoId.trim()
      ) {
        console.warn(
          "⚠️ Invalid companyName or qudemoId, skipping intro video fetch",
        );
        return;
      }

      console.log(
        "🎬 Fetching intro video preview with props (before qudemoData)...",
      );
      ...
      const fetchDirectIntroVideo = async () => {
        try {
          const response = await fetch(
            getVideoApiUrl(
              `/ask/${encodeURIComponent(companyName)}/${qudemoId}`,
            ),
            {
              method: "POST",
          ...
          console.error(
            "❌ Error fetching intro video preview (from props):",
            error,
          );
        }
      };
    ...
    try {
      ...
      if (data.videos && data.videos.length > 0) {
        ...
        if (firstVideo.thumbnail || firstVideo.poster || firstVideo.preview) {
          setVideoThumbnail(
            firstVideo.thumbnail || firstVideo.poster || firstVideo.preview,
          );
        } else if (videoUrl) {
  ...
  const loadBetaVersionData = async () => {
    try {
      ...
      if (!qudemoId) {
        ...
        console.log(
          "🎯 Widget: Skipping static video flow (qudemoId provided)",
        );
      }
      ...
      try {
        ...
        if (qudemoId && companyName) {
          console.log(
            "🎯 Widget: Loading specific QuDemo:",
            qudemoId,
            companyName,
          );
          const token = localStorage.getItem("accessToken");
          qudemoResponse = await fetch(
          ...
          qudemoResponse = await fetch(
            getNodeApiUrl(`/api/qudemos/share/${UNIVERSAL_DEMO_TOKEN}`),
          );
        }
        ...
        if (
          qudemoResponseData.success &&
          (qudemoResponseData.data || qudemoResponseData.qudemo)
        ) {
          ...
          let extractedCompanyName =
            companyName || qudemo.company?.name || qudemo.company_name;
          ...
          console.log(
            "✅ Widget: QuDemo loaded:",
            qudemo.title || qudemo.name,
            "- Company:",
      ...
      if (
        videoFlowData &&
        videoFlowData.videos &&
        videoFlowData.videos.length > 0
      ) {
          ...
          videoFlowData.videos.forEach((video) => {
            if (video.nextQuestions && staticQuestions.length < 6) {
              video.nextQuestions.forEach((q) => {
                if (
                  q.text &&
                  !staticQuestions.includes(q.text) &&
                  staticQuestions.length < 6
                ) {
                  staticQuestions.push(q.text);
                }
              });
      ...
      if (loadedQudemo && loadedQudemo.id && loadedQudemo.company_name) {
        try {
          const suggestedQuestionsUrl = getVideoApiUrl(
            `/suggested-questions/${encodeURIComponent(loadedQudemo.company_name)}/${loadedQudemo.id}`,
          );
          console.log(
            "🔍 Fetching suggested questions from:",
            suggestedQuestionsUrl,
          );

          const suggestedQuestionsResponse = await fetch(suggestedQuestionsUrl);
          console.log(
            "📡 Suggested questions response status:",
            suggestedQuestionsResponse.status,
          );

          const suggestedQuestionsData =
            await suggestedQuestionsResponse.json();
          ...
          if (questionsArray && questionsArray.length > 0) {
            qudemoQuestions = questionsArray
              .map((q) => {
      ...
      console.log(
        "✅ Suggested questions state updated with",
        questionsToSet.length,
        "questions",
      );
    } catch (error) {
  ...
  const handleSendMessage = async (messageText = null) => {
    ...
    if (isSalesRelated(userQuestion)) {
      ...
      try {
        const companyName =
          qudemoData.company_name || qudemoData.company?.name || "unknown";
        const response = await fetch(
          getVideoApiUrl(
            `/ask/${encodeURIComponent(companyName)}/${qudemoData.id}`,
          ),
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
    ...
    if (
      matchResult.matched &&
      matchResult.videoIndex !== null &&
      matchResult.videoIndex !== -1 &&
      ...
      setTimeout(() => {
        setChatMessages((prev) => [
          ...prev,
          {
            ...
            text:
              matchResult.answer ||
              "Let me show you a video that answers your question!",
          },
        ]);
    ...
    try {
      ...
      if (data && data.answer) {
        ...
        if (targetVideoUrl && qudemoData.videos) {
          ...
          const videoIndex = qudemoData.videos.findIndex(
            (v) =>
              v.video_url === targetVideoUrl ||
              v.video_url.includes(targetVideoUrl),
          );
          ...
          if (videoIndex !== -1) {
            ...
            setVideoFlow((prev) => {
              const exists = prev.videos.some(
                (v) => v.url === tempVideo.url || v.src === tempVideo.url,
              );
              if (!exists) {
                return {
                  ...prev,
            ...
            setTimeout(() => {
              ...
              const targetIndex = videoFlow.videos.findIndex(
                (v) => (v.url || v.src) === tempVideo.url,
              );
  ...
  if (!isExpanded) {
    return (
      <div className={`fixed ${positionClasses[position]} z-50`}>
        <button onClick={handleExpand} className="group relative">
          ...
          <div className="relative w-20 h-20 md:w-36 md:h-36 rounded-full overflow-hidden shadow-2xl border-4 border-white hover:border-blue-500 transition-all duration-300">
            ...
            <div className="absolute inset-0 bg-black bg-opacity-30 flex items-center justify-center group-hover:bg-opacity-50 transition-all">
              <svg
                className="w-8 h-8 md:w-16 md:h-16 text-white"
                fill="currentColor"
                viewBox="0 0 20 20"
              >
  ...
  return (
    <>
      ...
      <div
        className={`fixed inset-0 md:inset-auto md:${positionClasses[position]} z-50 transition-all duration-300 p-4 md:p-0 flex items-center justify-center md:block`}
      >
        {/* Minimized bar */}
        {isMinimized ? (
          ...
          <div
            className="bg-white rounded-xl shadow-2xl overflow-hidden flex flex-col md:flex-row w-full md:w-auto"
            style={{
          ...
          >
            {loading ? (
              ...
              <>
                ...
                >
                  ...
                  {currentAvatarVideo ? (
                    ...
                    <HybridVideoPlayer
                      ...
                      url={
                        videoFlow.videos[currentVideoIndex].url ||
                        videoFlow.videos[currentVideoIndex].src
                      }
                      width="100%"
                      height="100%"
                      controls={true}
                    ...
                    <div className="flex flex-col items-center justify-center text-white p-8">
                      ...
                      <p className="text-sm text-gray-400">
                        Ask a question to see the demo video
                      </p>
                    </div>
                  )}
                </div>
                ...
                >
                  {/* Chat header */}
                  <div className="bg-blue-600 text-white px-3 h-10 md:px-4 md:py-3 flex items-center gap-2 flex-shrink-0">
                    <svg
                      className="w-5 h-5"
                      fill="none"
                      stroke="currentColor"
                      ...
                      <>
                        {chatMessages.map((msg, i) => (
                          <React.Fragment key={i}>
                            ...
                            >
                              <div
                                className={`max-w-[85%] px-3 py-2 rounded-xl text-sm leading-relaxed text-left ${msg.type === "bot" ? "bg-white border border-gray-200 text-gray-800" : "bg-blue-600 text-white"}`}
                              >
                                {msg.text}
                              </div>
                            </div>
                  ...
                  <div
                    className="px-3 py-2 md:py-2 border-t bg-gray-50 flex-shrink-0"
                    style={{
                      paddingTop:
