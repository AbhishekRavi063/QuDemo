...
const QudemoPreview = ({ qudemo, onClose }) => {
  ...
  // Preload videos when QuDemo changes
  useEffect(() => {
    if (qudemo?.videos && qudemo.videos.length > 0) {
      const videoUrls = qudemo.videos.map(video => video.video_url);
      const startTimes = qudemo.videos.map(video => 0); // Preload from start
      
      // Preload videos in background
      videoCache.preloadVideos(videoUrls, startTimes).catch(error => {
