  ...
  const handlePreviewKnowledgeSource = async (source) => {
    ...
    try {
      ...
      const response = await fetch(
        getNodeApiUrl(`/api/knowledge/source/${source.id}/content`),
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
          ...
          if (qa_pairs && qa_pairs.length > 0) {
            qa_pairs.forEach((qa) => {
              content_chunks.push({
                ...
                url: qa.metadata?.url || "",
                last_updated: qa.metadata?.timestamp
                  ? new Date(qa.metadata.timestamp * 1000).toISOString()
                  : new Date().toISOString(),
                metadata: {
                  enhanced: qa.metadata?.enhanced || false,
                  extraction_method:
                    qa.metadata?.extraction_method || "AI-assisted parsing",
                  enhancement_method:
                    qa.metadata?.enhancement_method || "OpenAI GPT-4",
            ...
            content_chunks.forEach((chunk) => {
              ...
              if (chunk.title && chunk.content) {
                ...
                title =
                  ...
                  chunk.metadata.source_info ||
                  "Content Chunk";
                content = chunk.text;
                content_type = chunk.metadata.content_type || "general";
                difficulty_level =
                  chunk.metadata.difficulty_level || "beginner";
                tags = chunk.metadata.tags || [];
                url = chunk.metadata.url || "";
                last_updated =
                  chunk.metadata.last_updated || new Date().toISOString();
              } else {
                // Fallback
                title = "Content Chunk";
              ...
              knowledge_items.push({
                ...
                metadata: {
                  ...
                  enhancement_method:
                    chunk.metadata?.enhancement_method || "Semantic search",
                ...
                related_links:
                  chunk.related_links || chunk.metadata?.related_links || [],
              });
            });
          } else {
...
**Note:** This is a preview of the processed content. The actual scraped data is stored in the vector database and can be retrieved through the Q&A system.`;
          ...
          const updatedSource = {
            ...
            extracted_data: {
              ...
              enhanced_items: knowledge_items.filter(
                (item) => item.metadata.enhanced,
              ).length,
              by_content_type: {
                faq_complete: knowledge_items.filter(
                  (item) => item.content_type === "faq_complete",
                ).length,
                paragraph: knowledge_items.filter(
                  (item) => item.content_type === "paragraph",
                ).length,
                section: knowledge_items.filter(
                  (item) => item.content_type === "section",
                ).length,
                list: knowledge_items.filter(
                  (item) => item.content_type === "list",
                ).length,
                code_block: knowledge_items.filter(
                  (item) => item.content_type === "code_block",
                ).length,
                table: knowledge_items.filter(
                  (item) => item.content_type === "table",
                ).length,
              },
              by_difficulty: {
                beginner: knowledge_items.filter(
                  (item) => item.difficulty_level === "beginner",
                ).length,
                intermediate: knowledge_items.filter(
                  (item) => item.difficulty_level === "intermediate",
                ).length,
                advanced: knowledge_items.filter(
                  (item) => item.difficulty_level === "advanced",
                ).length,
              },
  ...
  const handleDeleteSource = async (sourceId) => {
    if (
      window.confirm(
        "Are you sure you want to delete this knowledge source? This action cannot be undone. This will permanently remove the data from the database and vector store.",
      )
    ) {
      setDeletingSource(sourceId);
      try {
        // Find the source to get its details for deletion
        const sourceToDelete = knowledgeSources.find(
        ...
        const token = localStorage.getItem("accessToken");
        const response = await fetch(
          getNodeApiUrl(`/api/knowledge/source/${sourceId}`),
          {
            method: "DELETE",
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        alert(
          `Failed to delete knowledge source: ${error.message}. Please try again or contact support if the problem persists.`,
        );
      } finally {
          ...
          {previewLoading ? (
            ...
            <>
                ...
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <p className="text-gray-600">
                      Data extracted from {selectedSource.source_url}
                    </p>
                  </div>
                  <div className="text-right">
                    <div className="text-3xl font-bold text-blue-600">
                      {selectedSource.extracted_data.total_items}
                    </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="text-center">
                    <div className="text-lg font-semibold text-green-600">
                      {selectedSource.extracted_data.enhanced_items}
                    </div>
                    <div className="text-xs text-gray-500">Enhanced</div>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-semibold text-blue-600">
                      {selectedSource.extracted_data.by_content_type
                        .faq_complete || 0}
                    </div>
                  ...
                  <div className="text-center">
                    <div className="text-lg font-semibold text-yellow-600">
                      {selectedSource.extracted_data.by_difficulty.beginner ||
                        0}
                    </div>
                  ...
                  <div className="text-center">
                    <div className="text-lg font-semibold text-purple-600">
                      {selectedSource.extracted_data.by_difficulty
                        .intermediate || 0}
                    </div>
              ...
              <div className="space-y-4">
                {selectedSource.extracted_data.knowledge_items &&
                selectedSource.extracted_data.knowledge_items.length > 0 ? (
                  selectedSource.extracted_data.knowledge_items.map(
                    (item, index) => (
                      <div
                        key={index}
                  ...
                  <div className="text-center text-gray-500 py-8">
                    ...
                    <p className="text-gray-600">
                      No knowledge items were extracted from this source. This
                      might be due to:
                    </p>
                    ...
                    <div className="mt-4 p-4 bg-gray-100 rounded-lg">
                      <p className="text-xs text-gray-600">
                        ...
                        detailed data structure and content chunks.
                      </p>
                    </div>
                  </div>
                )}
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600">
              Extracted and processed knowledge data for{" "}
              ...
              <div className="flex justify-between items-center">
                <div className="text-xs text-gray-500 space-y-1">
                  ...
                  <div>
                    Processed:{" "}
                    {source.processed_at
                      ? new Date(source.processed_at).toLocaleString()
                      : "Pending"}
                  </div>
                  <div>
      ...
      {knowledgeSources.length === 0 && (
        <div className="text-center text-gray-500 py-12">
          <div className="mb-4">
            ...
            <p className="text-gray-600 mb-4">
              Add website content, PDFs, and documents to enhance your AI
              assistant's knowledge base.
              <br />
              <span className="text-sm text-gray-500">
                (Videos are shown in the Videos tab)
              </span>
            </p>
          </div>
        </div>
